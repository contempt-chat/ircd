...
long	oline_flags_parse(char *string)
{
	...
	for (s = string; *s; s++)
	{
		...
		case 'A': tmp |= (ACL_ALL & 
            ...
            /* The possibility of hiding idletime (H) must be specified explicitly - not within A */
            tmp &= ~ACL_HIDEIDLE;
			break;
...
#ifdef SPOOF
int attach_conf_check_limits_spoof(aClient *cptr, aConfItem *aconf)
{
	int hcnt = 0, ucnt = 0;
	int ghcnt = 0, gucnt = 0;
	anUser *user = NULL;
	...
	**	host check is done on the spoofed host address.
	...
	*/
	...
	{
		/* note here that we are using name2 (the spoofed hostname) not the
		 * actual hostname or IP -- mh 2020-06-24 */
		for ((user = hash_find_hostname(aconf->name2, NULL)); user; user = user->hhnext)
		{
			if (!mycmp(aconf->name2, user->host))
			{
				ghcnt++;
...
int	attach_conf(aClient *cptr, aConfItem *aconf)
{
	...
	{
#ifdef SPOOF
		/* if this is a spoofed config, count hostnames matching the spoofed
		 * one only -- mh 2020-06-25 */
		if (IsConfSpoofed(aconf))
		{
			int i = attach_conf_check_limits_spoof(cptr, aconf);
		...
		else /* this is not a spoofed config, count ips matching its ip
		      * -- mh 2020-06-25 */
		{
#endif
		int hcnt = 0, ucnt = 0;
