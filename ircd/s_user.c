...
#endif
				(*buf) ? buf : "+",
                IsSASLAuthed(sptr) ? user->sasl_user : "*",
				sptr->info);
	}	/* for(my-leaf-servers) */
#ifdef	USE_SERVICES
#if 0
...
int	m_unick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	*/
	...
	if(parc == 9 && *parv[7] != '*' && *parv[7] != ':' /* old ircds still send realnames here */
	) {
        acptr->user->flags |= FLAGS_SASL;
        acptr->user->sasl_user = mystrdup(parv[7]);
    }
		...
		*/
		if (((IsServer(cptr) || IsService(cptr))
		    ...
		    if(IsBlockedNonAuthMsg(sptr, acptr)) {
                sendto_one(sptr, replies[ERR_MSGNEEDSASLAUTH], ME, BadTo(parv[0]), nick);
                continue;
		    }
